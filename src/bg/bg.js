    ...
    try {
      switch (message?.action) {
        case "getBgStoreDelta":
          promise = this.whenReady().then(() => this.bgStorePathLine.getDelta(message.id, message.patchId));
          break;
        case "getConfigStore":
          promise = this.whenReady().then(() => this.bgStore.config.toJSON());
          break;
        case "updateTorrentList":
          promise = this.whenReady().then(() => this.client.updateTorrents(message.force));
          break;
        case "start":
        ...
        case "setDownloadSpeedLimitEnabled":
          promise = this.whenReady().then(() => this.client.setDownloadSpeedLimitEnabled(message.enabled));
          break;
        case "setDownloadSpeedLimit":
          promise = this.whenReady().then(() => this.client.setDownloadSpeedLimit(message.speed));
          break;
        case "setUploadSpeedLimitEnabled":
          promise = this.whenReady().then(() => this.client.setUploadSpeedLimitEnabled(message.enabled));
          break;
        case "setUploadSpeedLimit":
          promise = this.whenReady().then(() => this.client.setUploadSpeedLimit(message.speed));
          break;
        case "setAltSpeedEnabled":
          promise = this.whenReady().then(() => this.client.setAltSpeedEnabled(message.enabled));
          break;
        case "setAltUploadSpeedLimit":
          promise = this.whenReady().then(() => this.client.setAltUploadSpeedLimit(message.speed));
          break;
        case "setAltDownloadSpeedLimit":
          promise = this.whenReady().then(() => this.client.setAltDownloadSpeedLimit(message.speed));
          break;
        case "updateSettings":
          promise = this.whenReady().then(() => this.client.updateSettings());
          break;
        case "sendFiles":
          promise = this.whenReady().then(() => this.client.sendFiles(message.urls, message.directory));
          break;
        case "getFreeSpace":
          promise = this.whenReady().then(() => this.client.getFreeSpace(message.path));
          break;
        case "reannounce":
